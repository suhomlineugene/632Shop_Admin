...
import { ApprovalsServiceProxy, OilApprovalDto } from '@shared/service-proxies/service-proxies';
...
@Component({
    templateUrl: './oil-approvals.component.html',
    styleUrls: ['./oil-approvals.component.scss'],
...
export class OilApprovalsComponent extends AppComponentBase implements OnInit {
    ...
    public getOilApprovals(event?: TableLazyLoadEvent): void {
        ...
        this._approvalsService.getAllOilApprovals()
            .pipe(finalize(() => {
    ...
    public deleteOilApproval(approval: OilApprovalDto): void {
        this.message.confirm(
            this.l('OilApprovalDeleteWarningMessage', approval.name),
            this.l('AreYouSure'),
            (isConfirmed) => {
                if (isConfirmed) {
    ...
    public createOrEditOilApproval(approval?: OilApprovalDto): void {
        const dialog: BsModalRef = this._modalService.show(CreateEditOilApprovalDialogComponent, {
            class: 'modal-lg',
            initialState: {
                id: approval?.id,
            },
    ...
    private _allRecords: OilApprovalDto[] = [];
